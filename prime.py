...
def get_primes(n):
    ...
    while number_range:                                                 # while loop that works through the range "number_range"
        ...
        number_range.difference_update(set(range(p * 2, n + 1, p)))     # uses "difference_update" to remove the non prime numbers from the array "number_range"
    return primes                                                       # returns the array of "primes"
